...
PHP_FUNCTION(phpd_transport_server)
{
	...
	phpd_transport_error_string = emalloc(1024);
	...
	for(zend_hash_internal_pointer_reset_ex(arr_hash, &pointer); zend_hash_get_current_data_ex(arr_hash, (void**) &data, &pointer) == SUCCESS; zend_hash_move_forward_ex(arr_hash, &pointer)) {
		...
		if (zend_hash_get_current_key_ex(arr_hash, &key, &key_len, &index, 0, &pointer) == HASH_KEY_IS_STRING) {
			...
			if(!strcmp(key, "local_cert")) {
				...
				phpd_transport_certfile = emalloc(Z_STRLEN(temp));
				...
				phpd_transport_passphrase = emalloc(Z_STRLEN(temp));
...
{
	int	retval = 0, headers_read = 0, headers_read_total = 0, content_length = 0, content_read_total = 0;
	char	tmpbuf[phpd_transport_max_length_header];

	memset(tmpbuf, 0, phpd_transport_max_length_header);
	phpd_transport_request = emalloc(phpd_transport_max_request + 1);
	...
	while(1)
	{
		...
		{
			phpd_transport_error_set("Request exceeded phpd_transport_max_request.");
			RETURN_FALSE;
		}
		...
		switch(SSL_get_error(phpd_transport_ssl, headers_read))
		{
			case SSL_ERROR_NONE:
			{
				memcpy(&phpd_transport_request[headers_read_total], &tmpbuf, headers_read);
		...
		{
			content_length = strtol(&tmpbuf[16], NULL, 10);
		}
	}
	...
	if(content_length>=1)
	{
		...
		{
			content_read = BIO_gets(phpd_transport_io, tmpbuf, (content_length-content_read)+1);
			memcpy(&phpd_transport_request[(headers_read_total+content_read_total)], &tmpbuf, content_read);
			content_read_total += content_read;
		}
	}

	//phpd_transport_request = erealloc(buf, headers_read_total + 1);
	phpd_transport_request[(headers_read_total + content_read_total + 1)] = '\0' ;

// set response code i.e. 408 - timeout if bad read

	RETURN_STRINGL(phpd_transport_request, headers_read_total + content_read_total, 0);
}
/* }}} */
